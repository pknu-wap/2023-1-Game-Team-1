...
void UseSkillClient()
{
	...
	local e = ActionStateChangedEvent("swingO1", "swingO1", 1.5 * self.playerComponent.atkSpeed, SpriteAnimClipPlayType.Onetime)
	...
	self.hitComponent.skillTimer2 = _TimerService:SetTimerOnce(self.UseSkillClient2, 0.5 - 0.5 * (self.playerComponent.atkSpeed - 1))
}

[Client]
void UseSkillClient2()
{
	local e = ActionStateChangedEvent("swingOF", "swingOF", 1.5 * self.playerComponent.atkSpeed, SpriteAnimClipPlayType.Onetime)
...
void UseSkillServer(Entity player, number delay)
{
	...
	_EffectService:PlayEffectAttached(self.effectRUID[1], player, Vector3.zero, 0, Vector3.one * 0.75, false, {FlipX = flip, PlayRate = player.ExtendPlayerComponent.atkSpeed})
