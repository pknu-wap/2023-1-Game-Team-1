...
void UseSkillClient()
{
	...
	if self.stateComponent.CurrentStateName == "JUMP" or self.stateComponent.CurrentStateName == "FALL" then
		...
		return
	elseif self.stateComponent.CurrentStateName == "IDLE" or self.stateComponent.CurrentStateName == "MOVE" or self.stateComponent.CurrentStateName == "ATTACK_WAIT" then
		self.stateComponent:ChangeState("SKILL")
...
void UseSkillServer(Entity player)
{
	...
	_EffectService:PlayEffect(self.effectRUID[1], player.Parent, player.TransformComponent.Position, 0, Vector3.one, false, {FlipX = flip})
}
