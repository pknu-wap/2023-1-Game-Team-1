...
void UseSkillServer(Entity player, number delay)
{
	...
	_TimerService:SetTimerOnce(function() 	
		player.AttackComponent:Attack(self.attackSize[1], self.attackOffset[1] * player.PlayerControllerComponent.LookDirectionX, "wa3", CollisionGroups.Player)
	end, delay)
}
...
void EnableBuff(Entity player)
{
	...
	log(player.Name.." : 공격력 버프 활성화")
}

[Server]
void DisableBuff(Entity player)
{
	log(player.Name.." : 공격력 버프 비활성화")
}
