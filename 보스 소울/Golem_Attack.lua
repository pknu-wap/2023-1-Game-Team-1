...
void OnBeginPlay()
{
	...
	elseif self.BossComponent.BossLevel == 2 then
		self.BossComponent.MaxHp = 8000
		self.BossComponent.Hp = self.BossComponent.MaxHp
		
	elseif self.BossComponent.BossLevel == 3 then
		self.BossComponent.MaxHp = 24000
		self.BossComponent.Hp = self.BossComponent.MaxHp
...
void StoneGolemAttack(number PtNo)
{
	...
	_SoundService:PlaySound("0069c5bfcf724a50b537f8844bd68c81", 1)
...
void IceGolemAttack(number PtNo)
{
	...
	if PtNo < 5 then
		_SoundService:PlaySound("267aa4a9324f460b80bc04600aa4d0e0", 1)
		...
		_SoundService:PlaySound("31a0879abfdb4fc897ab23ef37487088", 1)
...
{
	self.BossComponent.stateComponent:ChangeState("SKILL")
	_SoundService:PlaySound("98fbeec0bc974243bcd6625b5e3adf8a", 1)
	
	for i = 1,3 do
		local randomNum = math.random(-9, 8)
		
		local pos = Vector3(randomNum, self.BossComponent.BossTransformComponent.Position.y, 0)
