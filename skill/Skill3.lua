...
void UseSkill()
{
	...
	self.timerId = _TimerService:SetTimerOnce(function() self:CounterAttack(false) end, 0.75)
...
[Client]
void CounterAttack(boolean isSuccess)
{
	local e = ActionStateChangedEvent("swingTF", "swingTF", 1.0, SpriteAnimClipPlayType.Onetime)
	self.state:ChangeState("SKILL")
	_TimerService:SetTimerOnce(self.ChangeStateToIDLE, self.delay)
